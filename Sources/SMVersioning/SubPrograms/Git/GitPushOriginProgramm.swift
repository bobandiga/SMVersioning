...
struct GitPushOriginProgramm {
    ...
    enum LocalError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            switch self {
                case .git(let string):
                    return "Can`t push origin git tag version. \(string)"
                case .local:
                    return "Can`t push origin git tag version. Unknown error."
            }
        }
    }
    ...
    private let arguments = ["push", "origin"]
